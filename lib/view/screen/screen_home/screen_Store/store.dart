      ...
      builder: (controller) {
        return Scaffold(
          appBar: AppBar(
              ...
              actions: [
                if (controller.user.value!.sellerStatus?.isAccpeted == true)
                  MyIconBottun(
                    count: controller.sellerPurchases.fold(
                        0,
                        (value, purchase) =>
                            value + (purchase.status == 'waiting' ? 1 : 0)),
                    onTap: () => Get.toNamed(AppRoute.purchases),
                    icon: Icons.history,
                  ),
                const Gap(2),
                MyIconBottun(
                  count: controller.clientPurchases.fold(
                      0,
                      (value, purchase) =>
                          value + (purchase.status == 'waiting' ? 1 : 0)),
          ...
          floatingActionButton: controller
              ...
              : Align(
                  alignment:
                      Get.find<LocaleController>().language.languageCode == 'ar'
                          ? Alignment.bottomLeft
                          : Alignment.bottomRight,
                  child: FloatingActionButton(
                    backgroundColor:
                        controller.user.value!.sellerStatus?.isAccpeted == true
                    ...
                    children: [
                      ...
                      GetBuilder<StoreController>(
                        ...
                        builder: (controller) {
                          ...
                          return Container(
                            child: controller.productsStatusRequest ==
                                ...
                                : ListView.builder(
                                    ...
                                    itemBuilder: (context, indx) {
                                      return VewModel(
                                        buy: () {
                                          controller.buy(items[indx]);
                                        },
                                        product: items[indx],
                                        onTapcontry: controller.recting
                                            ? null
                                            : () =>
                                                controller.react(items[indx]),
